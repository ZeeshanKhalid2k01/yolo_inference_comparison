"""
Run YOLO (.pt) detection on a video and save the detection video.
Usage: python run_video_detect.py <input_video> [--model MODEL.pt] [--out OUTPUT.mp4] [--out-name NAME] [--conf 0.35] [--imgsz 640]
...
# project root: parent of python_yolo
...
def main():
    ap = argparse.ArgumentParser(description="YOLO .pt video detection")
    ap.add_argument("video", nargs="?", help="Input video path")
    ap.add_argument("--model", "-m", default="", help=".pt model name in yolo_models/ or full path")
    ...
    ap.add_argument("--show-cores", action="store_true", help="Overlay logical core count")
    args = ap.parse_args()
    ...
    # resolve model path
    if args.model:
        model_path = args.model if os.path.isabs(args.model) else os.path.join(YOLO_DIR, args.model)
    else:
        default_pt = os.path.join(YOLO_DIR, "yolo11n.pt")
        if os.path.isfile(default_pt):
            model_path = default_pt
        else:
            pts = [f for f in os.listdir(YOLO_DIR) if f.endswith(".pt")]
            if not pts:
                print("No .pt model in yolo_models/. Put a model there or use --model path.")
                sys.exit(1)
            model_path = os.path.join(YOLO_DIR, pts[0])
    ...
    if args.out:
        ...
        out_path = os.path.join(OUT_DIR, f"{base}_{model_stem}_pt_{ts}.mp4")
    ...
    try:
        while True:
            ...
            # draw on frame (Ultralytics plot is on a new image; we use boxes only for speed)
            vis = results.plot()
            if vis is None:
                vis = frame.copy()
